...
def main(): 
        ...
        if st.button("🔍 เริ่มตรวจสอบเอกสาร", type="primary"):
            with st.spinner("🔄 กำลังตรวจสอบเอกสาร กรุณารอสักครู่..."):
                try:
                    ...
                    # if error_stats:
                    #     st.subheader("💡 การวิเคราะห์และข้อเสนอแนะ")
                        
                    #     col1, col2 = st.columns(2)
                    #     with col1:
                    #         st.write("**🎯 ปัญหาหลักที่พบ:**")
                    #         top_3_errors = sorted(error_stats.items(), key=lambda x: x[1], reverse=True)[:3]
                    #         for i, (error_type, count) in enumerate(top_3_errors, 1):
                    #             st.write(f"{i}. {error_type} ({count} จุด)")
                        
                    #     with col2:
                    #         st.write("**🚀 ข้อเสนอแนะในการแก้ไข:**")
                    #         suggestions = []
                            
                    #         if any('ฟอนต์' in error for error in error_stats.keys()):
                    #             suggestions.append("ใช้ Find & Replace เพื่อเปลี่ยนฟอนต์ทั้งเอกสาร")
                    #         if any('ขนาด' in error for error in error_stats.keys()):
                    #             suggestions.append("ตั้งค่า Style ให้ถูกต้องตามประเภทเนื้อหา")
                    #         if any('อ้างอิง' in error for error in error_stats.keys()):
                    #             suggestions.append("ตรวจสอบรูปแบบและตำแหน่งการอ้างอิงตาราง/รูปภาพ")
                    #         if any('เยื้อง' in error for error in error_stats.keys()):
                    #             suggestions.append("ใช้ Paragraph formatting เพื่อตั้งค่าการเยื้อง")
                            
                    #         for suggestion in suggestions[:4]:  # แสดงไม่เกิน 4 ข้อ
    ...
    with st.expander("💡 เทคนิคการจัดรูปแบบเอกสารรายงานโครงงาน"):
        ...
        with col1:
            ...
            **⚖️ ขนาดฟอนต์ตามมาตรฐาน:**
            - หัวข้อหลัก (บทที่, สารบัญ): 18pt, Bold
    ...
    # ข้อมูลเกี่ยวกับ XML Template
    with st.expander("🔧 รายละเอียด XML Template ใหม่"):
        st.markdown("""
        **📋 โครงสร้างไฟล์ XML Template รุ่นใหม่:**
        
        ```xml
        <?xml version="1.0" encoding="UTF-8"?>
        <document_standards>
            <font>
                <theme_font>TH Sarabun New</theme_font>
            </font>
            <font_sizes>
                <!-- ขนาดฟอนต์ต่างๆ -->
            </font_sizes>
            <references>
                <picture_position>below_center</picture_position>
                <table_position>above_left</table_position>
                <reference_font_size>14</reference_font_size>
            </references>
            <!-- ... และส่วนอื่นๆ -->
        </document_standards>
        ```
        
        **🎛️ ส่วนใหม่ที่เพิ่มเข้ามา:**
        - **references**: กำหนดรูปแบบการอ้างอิงตารางและรูปภาพ
        - **cover_format_rules**: กำหนดรูปแบบหน้าปกแบบละเอียด
        - **valid_table_patterns**: รูปแบบการอ้างอิงตารางที่ถูกต้อง
        - **valid_picture_patterns**: รูปแบบการอ้างอิงรูปภาพที่ถูกต้อง
        - **specific_checks**: การตรวจสอบเฉพาะเจาะจง
        """)
    
    # Footer
    st.markdown("---")
    st.markdown(
        "<div style='text-align: center; color: gray;'>Document Checker for Project Reports v6.0 | Enhanced with Table & Picture References Support | Powered by Streamlit</div>",
